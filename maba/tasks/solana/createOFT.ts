...
const DEFAULT_LOCAL_DECIMALS = 2    
...
task('lz:oft:solana:create', 'Mints new SPL Token and creates new OFT Store account')
    ...
    .addOptionalParam('localDecimals', 'Token local decimals (default=2)', DEFAULT_LOCAL_DECIMALS, devtoolsTypes.int)
    ...
    .setAction(
        async ({
            ...
            const nonInteractive = ci || process.env.CI === 'true'
            ...
            if (onlyOftStore && !nonInteractive) {
            ...
            const confirmMaxSupply = nonInteractive || (await promptToContinue(maxSupplyStatement))
            if (!confirmMaxSupply) {
                return
            }
