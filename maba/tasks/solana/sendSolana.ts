...
export async function sendSolana({
    ...
    const tokenProgramPublicKey = new PublicKey(tokenProgramStr ?? TOKEN_PROGRAM_ID.toBase58())
    const tokenProgramId = fromWeb3JsPublicKey(tokenProgramPublicKey)
    ...
    const tokenAccount = fromWeb3JsPublicKey(tokenAccountAddress)
    logger.info(
        `Using token account ${tokenAccount} for owner ${umiWalletSigner.publicKey} with token program ${tokenProgramPublicKey.toBase58()}`
    )
    logger.info(`Using token mint ${mintPk.toBase58()}`)
    const balance = (await getAccount(connection, tokenAccountAddress, undefined, tokenProgramPublicKey)).amount

    // 5️⃣ Normalize human→base units
    const decimals = (await getMint(connection, mintPk, undefined, tokenProgramPublicKey)).decimals
